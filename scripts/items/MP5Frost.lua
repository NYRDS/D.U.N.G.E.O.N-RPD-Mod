...
BulNow=0
        ...
        return {
            ...
            name          = "Гадюка-5",
            info          = "Взяв этот пистолет-пулемёт ты почувствовал что он немного холодный, а также заметил возле ствольной накладки морозец, видимо туда поместили артефакт \"Снежинка\"",
...
local hero=RPD.Dungeon.hero
 ...
 if action == "ВЫСТРЕЛИТЬ" and cell ~= nil then
   ...
   RPD.affectBuff(enemy,RPD.Buffs.Slow,2)
   end
 ...
 for i=0,4 do
  ...
  if math.random(0,maxCh)>=missCh*1.2 then
    enemy:damage(math.random(minDmg+(3*thisItem:level()+1)-enemy:dr(),maxDmg+(3*thisItem:level()+1)-enemy:dr()), thisItem:getUser())
    else
